...
namespace Moritz.Spec
{
	public class ChainTrk : Trk
	{
			...
			: base(midiChannel)
		{
			...
			A.Assert(startPitches.Count == targetPitches.Count);
			for(int i = 0; i < startPitches.Count; i++)
			{
				A.Assert(startPitches[i] >= 0 && startPitches[i] <= 127);
				A.Assert((targetPitches[i] >= 0 && targetPitches[i] <= 127) || targetPitches[i] == int.MaxValue || targetPitches[i] == byte.MaxValue);
			}
		...
		private List<List<byte>> GetPitchesPerMCD(Envelope pitchEnv, int nMidiChordDefs, List<int> startPitches, List<int> targetPitches)
		{
			...
			A.Assert(envelope.Count == nMidiChordDefs + 1);
				...
				for(int pitchIndex = 0; pitchIndex < shearListsPerChain.Count; pitchIndex++)
				{
					...
					else
					{
						...
						if(shearListPerChord != null)
						{
							...
							byte pitch = A.SetRange0_127((int)Math.Round(startVal + envVal + shearIncrement));
							pitches.Add(pitch);
						}
					}
				}
