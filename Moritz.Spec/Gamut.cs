...
namespace Moritz.Spec
{
	...
	public class Gamut : ICloneable
	{
		...
		{
			#region checks
			A.AssertRange0_11(absPitchWeightDict.Keys);
			A.AssertRange0_127(absPitchWeightDict.Values);
			#endregion checks
		...
		{
			#region conditions
			A.Assert(absolutePitchWeights != null && absolutePitchWeights.Count > 1);
			foreach(var pitchWeight in absolutePitchWeights)
			{
				A.Assert(pitchWeight.Pitch >= 0 && pitchWeight.Pitch <= 11, "Absolute pitch out of range.");
				A.Assert(pitchWeight.Weight >= 1 && pitchWeight.Pitch <= 127, "Weight out of range.");
			}
			var exists = new List<int>();
			for(int i = 0; i < absolutePitchWeights.Count; i++)
			{
				A.Assert(!exists.Contains(absolutePitchWeights[i].Pitch), "Absolute pitches must be unique.");
				exists.Add(absolutePitchWeights[i].Pitch);
			}
			for(int i = 1; i < absolutePitchWeights.Count; i++)
			{
				A.Assert(absolutePitchWeights[i - 1].Weight >= absolutePitchWeights[i].Weight, "Weights must be in descending order.");
			}
		...
		private void SetPitchWeights()
		{
			...
			A.Assert(PitchWeights[0].Pitch >= 0 && PitchWeights[PitchWeights.Count - 1].Pitch <= 127, "Pitch out of range.");
			for(int i = 1; i < PitchWeights.Count; ++i)
			{
				A.Assert(PitchWeights[i].Pitch > PitchWeights[i - 1].Pitch, "Pitches must be unique and in ascending order.");
			}
			...
			foreach(var absPitch in absPitches)
			{
				...
				while(relPitch <= 127)
				{
					A.Assert(relPitches.FindIndex(x => x == relPitch) >= 0, "Each absolute pitch must occur in each possible octave.");
					relPitch += 12;
				}
			}
		...
		private void SetLinearChordShapesMatrix()
		{
			...
			_linearChordShapesMatrix = A.GetLinearMatrix(inversion0);
		}
		#endregion
		...
		public int Weight(int pitch)
		{
			...
			A.Assert(pitchWeight != defaultPitchWeight, $"Gamut.PitchWeights does not contain pitch {pitch}");
		...
		public int MinPitch
		{
			get
			{
				A.Assert(PitchWeights != null && PitchWeights.Count > 1);
	...
	public class StandardGamut : Gamut
	{
		...
		public StandardGamut(int pitchHierarchyIndex, int transposition, List<int> absolutePitches)
		{
			...
			A.AssertRange0_11(transposition);
			A.AssertRange0_11(absolutePitches);
			for(int i = 1; i < absolutePitches.Count; i++)
			{
				A.Assert(absolutePitches[i - 1] < absolutePitches[i]);
			}
			#endregion conditions
