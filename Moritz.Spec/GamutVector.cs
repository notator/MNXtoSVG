...
namespace Moritz.Spec
{
	...
	public class GamutVector
	{
		...
		{
			A.Assert(startGamut != null && targetGamut != null);
			A.Assert(pitchVectorEndPointsList != null && pitchVectorEndPointsList.Count >= 1);
			foreach(var tuple in pitchVectorEndPointsList)
			{
				A.Assert(tuple.Item1 >= startGamut.MinPitch && tuple.Item1 <= startGamut.MaxPitch);
				A.Assert(tuple.Item2 >= targetGamut.MinPitch && tuple.Item2 <= targetGamut.MaxPitch);
			}
			A.Assert(steps > 0);
		...
		{
			A.Assert(PitchWeightVectors.Count == concatenatedGamutVector.PitchWeightVectors.Count);
