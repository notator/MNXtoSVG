...
namespace Moritz.Spec
{
    ...
    public class InputChordDef : DurationDef, IUniqueSplittableChordDef
    {
        ...
        /// </summary>
        public InputChordDef(int msPositionReFirstIUD, int msDuration, List<InputNoteDef> inputNoteDefs, A.Dynamic dynamic, TrkOptions trkOptions)
            : base(msDuration)
        {
			...
			{
				A.Assert(ind.NotatedMidiPitch > pitchBelow);
				...
				if(ind.NoteOn != null && ind.NoteOn.SeqRef != null)
				{
					...
					{ 
						A.Assert(msPositionReFirstIUD <= trk.MsPosition);
					}
				}
				if(ind.NoteOff != null && ind.NoteOff.SeqRef != null)
				{
					...
					{
						A.Assert(minSeqPos <= trk.MsPosition);
					}
				}
			...
			_lyric = null;
            _dynamic = (dynamic == A.Dynamic.none) ? null : A.CLichtDynamicsCharacters[dynamic];
        ...
        public void Transpose(int interval)
        {
            ...
            {
				_inputNoteDefs[i].NotatedMidiPitch = A.SetRange0_127(_inputNoteDefs[i].NotatedMidiPitch + interval);
            }
        }
        ...
        public List<byte> NotatedMidiPitches
        {
            ...
            set
            {
                ...
                A.Assert(newPitches.Count == _inputNoteDefs.Count);
                int pitchBelow = -1;
                for(int i = 0; i < newPitches.Count; ++i)
                {
                    byte newPitch = newPitches[i];
                    A.Assert(newPitch > pitchBelow);
                    pitchBelow = newPitch;
        ...
        public List<byte> NotatedMidiVelocities
        {
            get
            {
                A.Assert(false, "Input Chords do not have notated Midi velocities.");
                return null;
            }
            set
            {
