    ...
    public class InputVoiceDef : VoiceDef
    {
            ...
            : base(midiChannel, msPositionReContainer, iuds)
        {
            foreach(IUniqueDef iud in UniqueDefs)
            {
                ...
                A.Assert(iud is InputChordDef || iud is InputRestDef);
            }
        }
		...
		public override void AssertConsistency()
        {
			...
			A.Assert(MidiChannel >= 0 && MidiChannel <= 3);	 // max 4 input channels

			A.Assert(Container == null || Container is Bar);

			if(Container is Bar)
			{
				foreach(IUniqueDef iud in UniqueDefs)
				{
					A.Assert(iud is InputChordDef || iud is InputRestDef || iud is CautionaryChordDef || iud is ClefDef);
				}
			}
        ...
        {
            A.Assert(inputVoiceDef is InputVoiceDef);
        ...
        {
            A.Assert(iUniqueDef is InputChordDef || iUniqueDef is InputRestDef || iUniqueDef is CautionaryChordDef || iUniqueDef is ClefDef);
        ...
        {
            A.Assert(!(Container is Bar), "Cannot Insert a Trk in a InputRestDef inside a Bar.");
            ...
            {
                A.Assert(false, "Can't find a rest spanning the chord!");
            }
        ...
        private void InsertVoiceDefInRest(int restIndex, VoiceDef iVoiceDef)
        {
            ...
            A.Assert(restStartMsPosReFirstIUD == replacementStartMsPosReFirstIUD && restEndMsPosReFirstIUD == replacementEndMsPosReFirstIUD);
        ...
        {
            A.Assert(originalRest is InputRestDef || originalRest is MidiRestDef);
            A.Assert(iVoiceDef[0] is MidiChordDef || iVoiceDef[0] is InputChordDef);
            A.Assert(iVoiceDef[iVoiceDef.Count - 1] is MidiChordDef || iVoiceDef[iVoiceDef.Count - 1] is InputChordDef);
        ...
        {
            A.Assert(inputVoiceDef[0] is InputChordDef && inputVoiceDef[inputVoiceDef.Count - 1] is InputChordDef);
        ...
        {
            A.Assert(!(replacementIUnique is MidiChordDef));
