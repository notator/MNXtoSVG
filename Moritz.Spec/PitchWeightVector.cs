...
namespace Moritz.Spec
{
	...
	public class PitchWeightVector
	{
		...
		{
			A.Assert(pitchVectorEndPoints.Item1 >= startGamut.MinPitch && pitchVectorEndPoints.Item1 <= startGamut.MaxPitch);
			A.Assert(pitchVectorEndPoints.Item2 >= targetGamut.MinPitch && pitchVectorEndPoints.Item2 <= targetGamut.MaxPitch);
			A.Assert(steps > 0);
		...
		{
			A.Assert(TargetPitchWeight == pitchWeightVectorToConcat.PitchWeights[0]);
