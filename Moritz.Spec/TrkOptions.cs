...
namespace Moritz.Spec
{
    ...
    public sealed class TrkOptions
    {
        ...
        public void WriteSVG(SvgWriter w, bool writeScoreNamespace)
        {
			...
			if(SpeedOption > 0)
			{
				w.WriteAttributeString("speed", SpeedOption.ToString(A.En_USNumberFormat));
			}
	...
	public class SpeedControl:TrkOption
	{
		...
		{
			A.Assert(speedFactor > 0, "Error: speedFactor must be greater than zero.");
	...
	public class VelocityTrkOption :TrkOption
	{
		...
		{
			A.Assert(minVelocity > 0 && minVelocity < 128);
